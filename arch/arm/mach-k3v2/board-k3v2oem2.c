...
#endif
#ifdef CONFIG_MATE_TP_TK_CMD_FEATURE
...
#ifdef CONFIG_MATE_TP_TK_CMD_FEATURE
#define MATE_SYNA_NAME "rmi_i2c_6cun"
#endif
//add for touchkey
...
static struct resource tibluesleep_resources[] = {
	{
		.name	= "host_wake",
		.start	= GPIO_HOST_WAKEUP,
		.end	= GPIO_HOST_WAKEUP,
...
static struct platform_device tibluesleep_device = {
	.name =	"bluesleep",
	.id		= -1,
	.num_resources	= ARRAY_SIZE(tibluesleep_resources),
	.resource	= tibluesleep_resources,
};
...
static struct synaptics_rmi4_platform_data syna_6cun_platformdata ={
    .regulator_en = true,
	.gpio = TOUCH_INT_PIN,
    .gpio_data = &rmi4_gpiodata,
...
static struct i2c_board_info hisik3_i2c_tp_devs[]= {
	...
	[0]	=	{
		...
		.addr			= 0x70,
		.flags 			= true,
		.platform_data 	= &syna_6cun_platformdata,
	},
#else
	[0]	=	{
        .type			= SYNA_NAME,
...
static void k3v2_i2c_devices_init(void)
{
	...
	if(get_spk_pa(str_tfa9887) == true) {
	    i2c_register_board_info(0, hisik3_i2c_bus0_tfa9887,
