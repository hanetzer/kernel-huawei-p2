...
static int stm_buffer_load(struct tty_struct *tty,
                     const unsigned char *buf, int count)
{
	int c = 0,ll = 0,ret = 0;
	char * p = 0;
	c = scan_ordinary(buf,count)+1;
	
	if (c > count) {//no 0x7e ppp flag	
	ret = mux_ringbuffer_write(&stmbuf,buf,count);
	}else {
//		printk("\n stm_buffer 0x7ep=%d\n",c);

		ll = mux_ringbuffer_avail(&stmbuf);
		
		ret = mux_ringbuffer_write(&stmbuf,buf,count);
		c = mux_ringbuffer_avail(&stmbuf);
	//	printk("\n stm_buffer: count=%d c=%d \n",count, c);
		mux_ringbuffer_read(&stmbuf,stm_tmp_buf,c,0);
		p = stm_tmp_buf;
		ret = mux_channel_write(tty, p, c);
	//	printk("\n stm_buffer_load mux_channel_write cnt=%d return=%d \n",c,ret);
		if (ret < c ){
			p += ret;
			if (ret >= ll){
				return (ret-ll);	
			}else {
				mux_ringbuffer_write(&stmbuf,p,ll-ret);
				return 0;
			}
		}
