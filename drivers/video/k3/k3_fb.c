...
u8 g_k3fb_in_suspend = 0;
u8 g_k3fb_in_resume = 0;
...
static void k3fb_te_dec(struct k3_fb_data_type *k3fd);
/******************************************************************************
** for HDMI
*/
...
void k3_fb_set_backlight(struct k3_fb_data_type *k3fd, u32 bkl_lvl)
{
	...
	/* CMD LCD May set fail, so recorder the value first and it will be handle in frame end work queue  */
	if (k3fd->panel_info.type == PANEL_MIPI_CMD) {
		k3fd->bl_level = bkl_lvl; 
	}
