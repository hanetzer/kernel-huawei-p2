...
#define CHARGE_FULL_TIME                (80) /*40min = 30s*80*/
#define DEBOUNCE_COUNT_USER_LOOP        (3)  /*(2160MA*1%)/(2000MA/60/60)=39s*/
#define DEBOUNCE_COUNT_FACTORY_LOOP     (2)  /*(1000MA*1%)/(2000MA/60/60)=18s*/
...
static int capacity_changed(struct bq_bci_device_info *di)
{
    ...
    */
    ...
    if(((di->charge_status == POWER_SUPPLY_STATUS_CHARGING) || (di->charge_status == POWER_SUPPLY_STATUS_FULL))
            && (curr_capacity > CHG_CANT_FULL_THRESHOLD)){
...
static int __devinit bq_bci_battery_probe(struct platform_device *pdev)
{
    ...
    /*
    * Debounce count is based on charging speed. In Factory Mode,
    * because Battery FW is not updated yet, default FCC is 1000mAh.
    * That's why it looks strange below
    */
    if(RUNMODE_FLAG_FACTORY == runmode_is_factory())
    {
        di->debounce_count = DEBOUNCE_COUNT_FACTORY_LOOP;
        dev_info(di->dev,"Factory Mode Debounce = %d\n",di->debounce_count);
    }
    else
    {
        di->debounce_count = DEBOUNCE_COUNT_USER_LOOP;
        dev_info(di->dev,"USER Mode Debounce = %d\n",di->debounce_count);
    }
