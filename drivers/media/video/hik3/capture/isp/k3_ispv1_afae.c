...
{
	return afae_ctrl->raw_unit_area;
}

void ispv1_set_stat_unit_area(u32 unit_area)
{
	afae_ctrl->raw_unit_area = unit_area;
}
...
static bool ispv1_setreg_vcm_code(u32 dest_code)
{
	...
	if ((dest_code < safe_position) && (curr_code > safe_position)
		...
		if (vcm->vcm_type == VCM_AD5823) {
			...
			max_step_cnt = 2;
		}  else if ((vcm->vcm_type == VCM_DW9714_SS  || vcm->vcm_type == VCM_DW9714_Sunny || vcm->vcm_type == VCM_DW9714_Liteon) && dest_code == vcm->offsetInit){
...
static int ispv1_setreg_vcm_lsc_mode(u16 para1, u16 para2)
{
	...
	print_debug("enter %s, para1:0x%x, para2:0x%x", __func__, para1, para2);
...
static int ispv1_focus_get_yuvrect_withzoom(camera_rect_s *yuv_rect, u32 zoom)
{
...
static int ispv1_set_focus_area_done(focus_area_s *area, u32 zoom)
{
	...
	if (multi_win == 0) {
		...
		ispv1_focus_get_yuvrect_withzoom(&cur_rect, zoom);
	...
	ispv1_focus_get_yuvrect_withzoom(&merged_yuv_rect, zoom);
...
int ispv1_set_ae_statwin(pic_attr_t *pic_attr)
{
	...
	x_offset = (pic_attr->in_width - raw.width * METERING_AECAGC_WINDOW_PERCENT / 100) / 2;
	y_offset = (pic_attr->in_height - raw.height * METERING_AECAGC_WINDOW_PERCENT / 100) / 2;
...
{
	metering_area_s *metering_area;
	print_debug("enter %s", __func__);

	if(!this_ispdata->sensor->af_enable)
		return 0;

	metering_area = &afae_ctrl->ae_area;
...
int ispv1_focus_status_collect(focus_frame_stat_s *curr_data, focus_frame_stat_s *mean_data)
{
	...
	int stat_cnt;
	int interval_threshold = DEFAULT_AF_GSENSOR_INTERVAL_THRESHOLD;

	if(this_ispdata->sensor->get_af_param)
		interval_threshold = this_ispdata->sensor->get_af_param(AF_GSENSOR_INTERVAL_THRESHOLD);
	...
	if ((curr_data->fps <= 20) || (afae_ctrl ->gsensor_interval <= interval_threshold))
...
int ispv1_copy_preview_data(u8 *dest, camera_rect_s *rect)
{
	...
	if (0 == frame->phyaddr) {
		print_error("params error, frame->phyaddr 0x%x", (unsigned int)frame->phyaddr);
		return -1;
	}
...
void ispv1_k3focus_run(void)
{
	...
	while ((schedule_case = ispv1_focus_need_schedule()) != -1) {
		...
		case FOCUS_SCHEDULE_CASE_VAF_MOVE:
			ispv1_copy_preview_data(pstat_data, rect);
...
static af_run_stage ispv1_af_recognise_curve(af_run_param *af_info, pos_info *curr, vcm_info_s *vcm)
{
	...
	if (curr->contrast <= get_af_judge_threshold_low(top->contrast) ||
		...
		af_get_next_vcmcode(trip, curr->code, trip->direction, AF_CODE_STEP_COARSE, &next_code);
		trip->step_cnt++;
		curr->code = next_code;
	} else {
