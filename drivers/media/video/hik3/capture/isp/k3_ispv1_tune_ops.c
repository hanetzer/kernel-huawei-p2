...
int ispv1_get_actual_iso(void)
{
	...
	print_info("%s 0x%x", __func__, gain);
...
int ispv1_get_exposure_time(void)
{
	...
	print_info("%s 0x%x", __func__, expo);
...
void set_iso_double_or_normal(camera_iso_double value)
{
	...
	camera_sensor *sensor;
	sensor = this_ispdata->sensor;
	int max_iso, min_iso, max_iso_lowlight;
	int max_gain, min_gain;
	...
	max_iso = sensor->get_override_param(OVERRIDE_ISO_HIGH);
	min_iso = sensor->get_override_param(OVERRIDE_ISO_LOW);
	max_iso_lowlight = sensor->get_override_param(OVERRIDE_ISO_HIGH_LOWLIGHT);

	if (value == CAMERA_ISO_DOUBLE)
		max_iso = max_iso_lowlight;
...
int ispv1_set_hw_lowlight(int ctl)
{
	...
	if(sensor->support_hw_lowlight)
	{
		...
		{
			print_error("sensor [%s] don't support lowlight", sensor->info.name);
			return -1;
		}
	}
	...
	if(ctl == 1)
	{
		...
		else if(sensor->init_isp_reg)
		{
			sensor->hw_lowlight_on = true;
			sensor->init_isp_reg();
		}
	}
	else if(ctl == 0)
	{
		if(hw_lowlight_flg == false)
		{
...

{
	...
	u8 ydenoise_value[ ] = {
		...
		ISP_YDENOISE_COFF_32X,
		ISP_YDENOISE_COFF_64X,
		};
   u16 uvdenoise_value[ISP_DENOISE_ARRAY_SIZE] = {
	0x02, /* gain 1x uv denoise */
	0x04, /* gain 2x uv denoise */
	0x08, /* gain 4x uv denoise */
	0x10, /* gain 8x uv denoise */
	0x20, /* gain 16x uv denoise */
	0x20,
	0x20,
	};
   ...
   u16 uvdenoise_value[ISP_DENOISE_ARRAY_SIZE] = {
          0x04, /* gain 1x uv denoise */
          0x04, /* gain 2x uv denoise */
          0x0C, /* gain 4x uv denoise */
          0x10, /* gain 8x uv denoise */  
          0x20, /* gain 16x uv denoise */
          0x40,
          0x50,
