...
#define INIT_MAX_TIMES 3   /*max times of dhd module init*/
#if defined(SOFTAP)
...
#else
#define DHD_REGISTRATION_TIMEOUT  4000  /* msec : allowed time to finished dhd registration */
#endif
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	sprintf(filter_rule, "100 0 0 0 0xffffffffffff 0x%02x%02x%02x%02x%02x%02x",
                        dhd->mac.octet[0], dhd->mac.octet[1], dhd->mac.octet[2],
			dhd->mac.octet[3], dhd->mac.octet[4], dhd->mac.octet[5]);
	dhd->pktfilter[0] = filter_rule;
...
dhd_module_init(void)
{
	...
	DHD_TRACE(("%s: Enter\n", __FUNCTION__));
	for (try_times = 0; try_times < INIT_MAX_TIMES; try_times++) {
		error = 0;

		DHD_ERROR(("%s: Try to init dhd module %d times\n", __FUNCTION__, try_times+1));
...
fail_1:
	...
	printk(KERN_ERR "%s: faild, error=%d\n", __FUNCTION__, error);
...
dhd_softap_configuation_t g_dhd_softap_configuration;

/* when set softap parameter, we store it.
 * we don't store it again unless softap open again.
 * WLC_SET_SSID is the lasest softap parameter to set */
void
dhd_store_softap_configuration(wl_ioctl_t *ioc, int cmd)
{
	if (g_dhd_softap_configuration.config_done_flag)
		return;

	if (strstr(fw_path, "_apsta") != NULL) {
		if (WLC_SET_SSID == cmd) {
			memcpy(g_dhd_softap_configuration.ssid, ioc->buf, ioc->len);
			g_dhd_softap_configuration.ssid_len = ioc->len;
			g_dhd_softap_configuration.config_done_flag = 1;
		} else if (WLC_SET_VAR == cmd) {
			if(!strcmp("wsec", ioc->buf)) {
				memcpy(g_dhd_softap_configuration.wsec, ioc->buf, ioc->len);
				g_dhd_softap_configuration.wsec_len = ioc->len;
				g_dhd_softap_configuration.security_flag = 1;
			} else if(!strcmp("wsec_key", ioc->buf)) {
				memcpy(g_dhd_softap_configuration.wsec_key, ioc->buf, ioc->len);
				g_dhd_softap_configuration.wsec_key_len = ioc->len;
				g_dhd_softap_configuration.security_flag = 0;
			} else if(!strcmp("auth", ioc->buf)) {
				memcpy(g_dhd_softap_configuration.auth, ioc->buf, ioc->len);
				g_dhd_softap_configuration.auth_len = ioc->len;
			} else if(!strcmp("wpa_auth", ioc->buf)) {
				memcpy(g_dhd_softap_configuration.wpa_auth, ioc->buf, ioc->len);
				g_dhd_softap_configuration.wpa_auth_len = ioc->len;
			}
		}
	}
	return;
...
/* when firmware need to be reloaded, we should set softap parameter to firmware again. */
void
dhd_reinit_softap_configuration(struct net_device *dev)
{
	uint32 ret = 0;
	struct wl_ioctl ioc;
	uint32 ap = 1;
	...
	if (strstr(fw_path, "_apsta") != NULL) {
		...
		ioc.set = 1;
		ioc.buf = &ap;
		ioc.len = sizeof(uint32);
		ret = dhd_ioctl_entry_local(dev, &ioc, ioc.cmd);
		...
		if (g_dhd_softap_configuration.security_flag) {
			memset(&ioc, 0, sizeof(ioc));
			ioc.cmd = WLC_SET_VAR;
			ioc.set = 1;
			ioc.buf = g_dhd_softap_configuration.wsec;
			ioc.len = g_dhd_softap_configuration.wsec_len;
			ret = dhd_ioctl_entry_local(dev, &ioc, ioc.cmd);

			memset(&ioc, 0, sizeof(ioc));
			ioc.cmd = WLC_SET_VAR;
			ioc.set = 1;
			ioc.buf = g_dhd_softap_configuration.wsec_key;
			ioc.len = g_dhd_softap_configuration.wsec_key_len;
			ret = dhd_ioctl_entry_local(dev, &ioc, ioc.cmd);

			memset(&ioc, 0, sizeof(ioc));
			ioc.cmd = WLC_SET_VAR;
			ioc.set = 1;
			ioc.buf = g_dhd_softap_configuration.auth;
			ioc.len = g_dhd_softap_configuration.auth_len;
			ret = dhd_ioctl_entry_local(dev, &ioc, ioc.cmd);

			memset(&ioc, 0, sizeof(ioc));
			ioc.cmd = WLC_SET_VAR;
			ioc.set = 1;
			ioc.buf = g_dhd_softap_configuration.wpa_auth;
			ioc.len = g_dhd_softap_configuration.wpa_auth_len;
			ret = dhd_ioctl_entry_local(dev, &ioc, ioc.cmd);
		}
		...
		ioc.set = 1;
		ioc.buf = g_dhd_softap_configuration.ssid;
		ioc.len = g_dhd_softap_configuration.ssid_len;
		ret = dhd_ioctl_entry_local(dev, &ioc, ioc.cmd);
	}
...
int dhd_ioctl_entry_local(struct net_device *net, wl_ioctl_t *ioc, int cmd)
{
	...
	DHD_OS_WAKE_UNLOCK(&dhd->pub);
#ifdef WL_PROTECT
	dhd_store_softap_configuration(ioc, cmd);
#endif
