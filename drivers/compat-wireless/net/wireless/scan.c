...
static void __cfg80211_bss_print_ssid(struct cfg80211_internal_bss *bss)
{
    const u8 *ssidie;
    char ssidstr[32+1];
    ...
    ssidie = cfg80211_find_ie(WLAN_EID_SSID,
                              bss->pub.information_elements,
                              bss->pub.len_information_elements);

    if (ssidie != NULL && ssidie[1] !=0 && ssidie[1] < 33 ) {
        memcpy(ssidstr, ssidie+2, ssidie[1]);
        printk(KERN_EMERG" ssid: %s\n", ssidstr);
    }
...
void cfg80211_bss_expire(struct cfg80211_registered_device *dev)
{
	...
	list_for_each_entry_safe(bss, tmp, &dev->bss_list, list) {
		...
		if (!time_after(jiffies, bss->ts + IEEE80211_SCAN_RESULT_EXPIRE))
			continue;
                printk(KERN_EMERG"%s: expired to unlink bss ", __func__ );
                __cfg80211_bss_print_ssid(bss);
			 ...
			 int force)
{
	...
	if (res->pub.beacon_ies && (force || !prev->pub.beacon_ies)) {
        ...
        /* print updated SSID */
        __cfg80211_bss_print_ssid(prev);
}
