...
void SDHUB_clock_disable( void ) {
    ...
    {
        clk_disable(pclk);
        printk(KERN_DEBUG "SDHUB_clock_disable.\n");
    }
...
void SDHUB_clock_enable( void ) {
    ...
    if(clock_state == 0)
    {
        ...
        udelay(50);
        clock_state = 1;
        printk(KERN_DEBUG "SDHUB_clock_enable.\n");
    }
}
...
static void mn66831_ctrl_host_power( struct mn66831_slot *slot_info, bool toggle ) 
{
	...
	Sdhubs_DebugPrint(__LINE__, SDHUB_PDEBUG,"%s : toggle = %d\n", __func__, toggle );
    ...
    if (toggle)
    {
        ...
        SDHUB_clock_enable();
		ret = 0 ;
    	Sdhubs_DebugPrint(__LINE__, SDHUB_PDEBUG,"%s:clk_enable start \n", __func__ );
        if (ret)
        {
    ...
    else
    {
    	...
    	Sdhubs_DebugPrint(__LINE__, SDHUB_PDEBUG,"%s:All Power Supply Off\n", __func__ );
    }
}
...
static void mn66831_ctrl_card_voltage( struct mn66831_slot *slot_info, unsigned int vol ) {
	...
	Sdhubs_DebugPrint(__LINE__, SDHUB_PDEBUG,"MMC mn66831_ctrl_card_voltage() -> %d\n", vol);
...
//		mn66831_reg_ordata( slot_info, MN66831_UHS1_REGSEL, 0x0007 );
    ...
    mn66831_reg_getdata(slot_info, MN66831_UHS1_REGSEL, &regdata );
    Sdhubs_DebugPrint(__LINE__, SDHUB_PDEBUG,"%s:MN66831_UHS1_REGSEL=0x%08x\n",__func__, regdata );
...
//    }
	...
	if( err_tmp != SDD_SUCCESS ) {
		Sdhubs_DebugPrint(__LINE__, SDHUB_PDEBUG,"%s:mn66831_cmd_sd_path_send_response=0x%X\n", __func__, err_tmp );
		return ;
	}
...
static void mn66831_set_ios( struct mmc_host *mmc, struct mmc_ios *ios ) {
	...
	Sdhubs_DebugPrint(__LINE__, SDHUB_PDEBUG,"%s:ios->power_mode = %u\n", __func__, ios->power_mode );
	Sdhubs_DebugPrint(__LINE__, SDHUB_PDEBUG,"%s:ios->bus_width = %u\n", __func__, ios->bus_width );
	Sdhubs_DebugPrint(__LINE__, SDHUB_PDEBUG,"%s:ios->clock = %u\n", __func__, ios->clock );
...
static int mn66831_suspend( struct platform_device *pdev, pm_message_t state ) {
	...
	Sdhubs_DebugPrint(__LINE__, SDHUB_PDEBUG,"%s: host_info->num_slots = %d  package->flag_connection= %d sdcard_init_status = %d\n",
