...
#define lfprintf(format, arg...)   printk("IPTH Freq gov msg: "format, ##arg)
...
const speed_freq_level_t  speed_freq_level[4] =
{
    ...
    {120000, 1500000, 450000},
};

#define  MAX_SPEED_FREQ_LEVEL            4
...
static void k3v2_release_freq_lock_work(struct work_struct *work)
{
    ...
    if(NULL == fl_control_ptr){
        printk("IPTH k3v2_release_freq_lock_work  NULL point error!\n");     
        return;
    }
...
static void k3v2_release_freq_lock_timer(unsigned long data)
{
    ...
    if(NULL == fl_control_ptr){
        printk("IPTH k3v2_release_freq_lock_timer  NULL point error!\n");
        return;
    }
...
static void  k3v2_do_freq_lock_work(struct work_struct *work){
    ...
    if((req_lock_level > MAX_SPEED_FREQ_LEVEL) || 
        (0 == req_lock_level)){
        printk("IPTH k3v2_do_freq_lock_work  invalid lock_level error!\n");
        return;
    }
...
Return  Value  :
******************************************************************************/
static int __init  k3v2_wifi_freq_ctl_init(void){
#ifdef CONFIG_CPU_FREQ_GOV_K3HOTPLUG        
    ...
    if (!freq_lock_control_ptr){
        printk("IPTH k3v2_wifi_freq_ctl_init  kzalloc  error!\n");
        goto cleanup;
    }
    ...
    if (!freq_lock_control_ptr->freq_ctl_wq){
        printk("IPTH k3v2_wifi_freq_ctl_init  create_singlethread_workqueue  error!\n");
    ...
    mutex_unlock(&freq_lock_control_ptr->lock_freq_mtx);
    printk("IP throughput monitor init ok! v2_2\n");
    return 0;
err_wq:
...
Return  Value  :
******************************************************************************/
static int __exit k3v2_wifi_freq_ctl_destroy(void)
{
#ifdef CONFIG_CPU_FREQ_GOV_K3HOTPLUG        
    ...
    * must release the lock directly here!*/
    ...
    kfree(freq_lock_control_ptr);
    freq_lock_control_ptr = NULL;
	printk("IP throughput monitor exit ok!\n");
#endif   /*end of  #ifdef CONFIG_CPU_FREQ_GOV_K3HOTPLUG*/
    return 0;
}
    ...
    */
    ...
    if(level_idx > 0){
        if(NULL == freq_lock_control_ptr){
            printk("IPTH k3v2_speed_calc_process  NULL point error!\n");
            return -1;
        }
...
EXPORT_SYMBOL(k3v2_wifi_txrx_count);

MODULE_AUTHOR("d00138378");
MODULE_DESCRIPTION("throughput monitor and CPU DDR frequency control");
MODULE_LICENSE("GPL");
module_init(k3v2_wifi_freq_ctl_init);
module_exit(k3v2_wifi_freq_ctl_destroy);
