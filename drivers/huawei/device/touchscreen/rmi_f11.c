...
#define TP_Y_MAX        1280
#define TP_Y_MARGIN     60
...
extern atomic_t tk_is_pressed;
extern u32 synaptics_release_time;
					...
					u8 finger_state, u8 n_finger)
{
	...
	if (prev_state && !finger_state) {
		...
		*/
		...
		axis_align_value = get_touchscreen_axis_align();

		switch(axis_align_value){
			case FLIP_X_FALSE_Y_FALSE:
				break;
			case FLIP_X_FALSE_Y_TRUE:
				axis_align->flip_y = ~ (axis_align->flip_y);
				break;
			case FLIP_X_TRUE_Y_FALSE:
				axis_align->flip_x = ~ (axis_align->flip_x);
				break;
			case FLIP_X_TRUE_Y_TRUE:
				axis_align->flip_x = ~ (axis_align->flip_x);
				axis_align->flip_y = ~ (axis_align->flip_y);
				break;
			default:
		...
		*/
		...
		if (axis_align->clip_Y_high)
			y =  min(axis_align->clip_Y_high, y);
                ...
                if (y > (TP_Y_MAX - TP_Y_MARGIN)) {
			if (atomic_read(&tk_is_pressed) == 1) {
				pr_info("%s: tk_is_pressed = 1\n", __func__);
				return;
			} else {
				t = (u32)ktime_to_ms(ktime_get());
				deltT = t - synaptics_release_time;
				if (deltT < 120) {
					pr_info("%s: deltT[%d] < 120\n", __func__, deltT);
					return;
				}
			}
                }
...
static int rmi_f11_set_tk_sensitivity(struct rmi_function_container *fc)
{
	...
	int rc;
	u8 glove_mode = 0;
	enum rmi_tp_status tp_status = TP_NULL;
	...
	if (tp_status != TP_NULL) {
            synaptics_set_sensitivity((uint8_t)tp_status);
	}
        return 0;
}
