...
static int rmi_f01_suspend(struct rmi_function_container *fc)
{
	...
	int retval = 0;
	struct rmi_phys_device *phys = rmi_dev->phys;
	//atomic_set(&touch_is_pressed, 0);

	dev_info(&fc->dev, "%s : phys_atten[%d], f11_irq[%d]\n", __func__,
		phys->info.attn_count,
		rmi_dev->irq_numbers);
	if (gpio_get_value(rmi_dev->irq_gpio_number) == 0) {
		dev_err(&fc->dev, "%s:TP's irq is low!\n", __func__);
	}
