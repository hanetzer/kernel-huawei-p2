				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (val == 1) {
		...
		if (data->enable_ps_sensor == 0) {
			...
			if(E_APDS_TYPE_Mate==apds_type ){
				apds990x_set_ppcount(client, 0x03);/* 3-Pulse for proximity*/
			}else{
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (val == 1) {
		...
		if (data->enable_als_sensor == 0) {
			...
			if(E_APDS_TYPE_Mate==apds_type ){
				apds990x_set_ppcount(client, 0x03);/* 3-pulse */
			}else{
				   ...
				   const struct i2c_device_id *id)
{
	...
	int err = 0;
       ...
       else if(E_APDS_TYPE_Mate==apds_type )
       {
            apds_990x_pwave_value = 300;
            threshold_value = 400;
       }
       else
        {        
